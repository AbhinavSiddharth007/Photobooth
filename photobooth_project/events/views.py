...
def create_event(request):
    ...
    event = Event.objects.create(
        name=event_name, owner_email=owner_email if owner_email else None
    )

    guest_url = request.build_absolute_uri(
        reverse("guest_gallery", args=[event.guest_code])
    )
    owner_url = request.build_absolute_uri(
        reverse("owner_dashboard", args=[event.owner_secret])
    )
    event.save()
...
def toggle_uploads(request, owner_secret):
    ...
    event.save()
    return JsonResponse({"success": True, "uploads_enabled": event.uploads_enabled})
